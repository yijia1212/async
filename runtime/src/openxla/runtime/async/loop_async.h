...
#ifndef OPENXLA_RUNTIME_ASYNC_LOOP_ASYNC_H_
#define OPENXLA_RUNTIME_ASYNC_LOOP_ASYNC_H_
...
static inline iree_loop_t iree_loop_async(iree_status_t* out_status) {
  iree_loop_t loop = {out_status, iree_loop_async_ctl};
  return loop;
}

// Minimum size in bytes required for iree_loop_async_storage_t.
...
#define IREE_LOOP_ASYNC_STORAGE_SIZE 512

// Storage for an async loop.
...
typedef iree_alignas(iree_max_align_t) struct iree_loop_async_storage_t {
  uint8_t opaque[IREE_LOOP_ASYNC_STORAGE_SIZE];
  iree_status_t status;
} iree_loop_async_storage_t;

// Returns an async loop that uses an external |storage| instead of the stack.
...
//
// See iree_loop_async for details on the execution behavior.
static inline iree_loop_t iree_loop_async_initialize(
    iree_loop_async_storage_t* storage) {
  storage->status = iree_ok_status();
  iree_loop_t loop = {
      storage,
      iree_loop_async_using_storage_ctl,
  };
  return loop;
}

static void iree_loop_async_deinitialize(iree_loop_async_storage_t* storage) {
  if (!storage) return;
...
#endif  // OPENXLA_RUNTIME_ASYNC_LOOP_ASYNC_H_
