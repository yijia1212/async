...
static iree_status_t iree_loop_async_reentrant_ctl(void *self,
                                                   iree_loop_command_t command,
                                                   const void *params,
                                                   void **inout_ptr);

static void iree_loop_async_emit_error(iree_loop_t loop, iree_status_t status);
...
static void iree_loop_async_run_call(iree_loop_t loop,
...
static void iree_loop_async_run_dispatch(iree_loop_t loop,
  ...
  for (uint32_t z = 0; z < workgroup_count_z; ++z)
  {
    for (uint32_t y = 0; y < workgroup_count_y; ++y)
    {
      for (uint32_t x = 0; x < workgroup_count_x; ++x)
      {
        ...
        if (!iree_status_is_ok(workgroup_status))
          goto workgroup_failed;
      }
    }
  }
workgroup_failed:
...
static void iree_loop_async_run_wait_until(
...
static void iree_loop_async_run_wait_one(iree_loop_t loop,
                                         iree_loop_wait_one_params_t params)
{
  ...
  if (params.wait_source.ctl == iree_async_token_wait_source_ctl)
  {
    ...
    // asyn_token_wait
    iree_status_t status = iree_async_token_and_then(token, params.callback, loop);
    if (!iree_status_is_ok(status))
    {
      iree_loop_async_emit_error(loop, status);
    }
...
static void iree_loop_async_run_wait_any(
...
static void iree_loop_async_run_wait_all(
    iree_loop_t loop, iree_loop_wait_multi_params_t params)
{
  IREE_TRACE_ZONE_BEGIN(z0);
...
#define IREE_LOOP_ASYNC_RING_CAPACITY ((uint8_t)8)
static_assert((IREE_LOOP_ASYNC_RING_CAPACITY &
               (IREE_LOOP_ASYNC_RING_CAPACITY - 1)) == 0,
...
//   uint8_t physical_idx = logical_idx % IREE_LOOP_ASYNC_RING_CAPACITY;
// or this, way better (though the compiler can usually figure it out):
//   uint8_t physical_idx = logical_idx & IREE_LOOP_ASYNC_RING_CAPACITY;
#define IREE_LOOP_ASYNC_RING_MASK (IREE_LOOP_ASYNC_RING_CAPACITY - 1)

// An operation in the async loop ringbuffer containing all the information
// required to replay it at a future time. All pointers are unowned.
typedef struct iree_loop_async_op_t
{
  iree_loop_command_t command;
  union
  {
...
// The async loop is explicitly not designed for multi-program cooperative
// scheduling and well-formed programs shouldn't hit the limit.
//
// NOTE: this structure must be in an initialized state if zeroed.
typedef struct iree_loop_async_ring_t
{
  iree_loop_async_op_t ops[IREE_LOOP_ASYNC_RING_CAPACITY];
  uint8_t read_head;
  uint8_t write_head;
  iree_status_t *status_ptr;
} iree_loop_async_ring_t;
static_assert(
    sizeof(iree_loop_async_ring_t) <= IREE_LOOP_ASYNC_STORAGE_SIZE,
    "iree_loop_async_ring_t needs to be tiny as it's allocated on the stack");

// Returns a loop that references the current ringbuffer for reentrant usage.
static inline iree_loop_t iree_loop_async_reentrant(
    iree_loop_async_ring_t *ring)
{
  iree_loop_t loop = {
      .self = ring,
      .ctl = iree_loop_async_reentrant_ctl,
  };
  return loop;
}
...
static inline void iree_loop_async_ring_initialize(
    iree_status_t *status_ptr, iree_loop_async_ring_t *out_ring)
{
...
static inline bool iree_loop_async_ring_is_empty(
    const iree_loop_async_ring_t *ring)
{
  return ring->read_head == ring->write_head;
}

// Returns true if the ringbuffer is full (write has caught up to read).
static inline bool iree_loop_async_ring_is_full(
    const iree_loop_async_ring_t *ring)
{
  return ((ring->write_head - ring->read_head) & IREE_LOOP_ASYNC_RING_MASK) ==
         IREE_LOOP_ASYNC_RING_MASK;
}
...
static iree_status_t iree_loop_async_enqueue(iree_loop_async_ring_t *ring,
                                             iree_loop_command_t command,
                                             const void *params)
{
  ...
  if (iree_loop_async_ring_is_full(ring))
  {
    return iree_make_status(
        IREE_STATUS_RESOURCE_EXHAUSTED,
        "async ringbuffer capacity exceeded; reduce the amount of concurrent "
        "work or use a real loop implementation");
  }

  // Reserve a slot for the new operation.
  uint8_t slot = ring->write_head;
  ring->write_head = (ring->write_head + 1) & IREE_LOOP_ASYNC_RING_MASK;
...
static void iree_loop_async_dequeue_and_run_next(
    iree_loop_async_ring_t *ring)
{
  IREE_ASSERT(!iree_loop_async_ring_is_empty(ring));

  // Acquire the next operation.
  uint8_t slot = ring->read_head;
  ring->read_head = (ring->read_head + 1) & IREE_LOOP_ASYNC_RING_MASK;

  // Copy out the parameters; the operation we execute may overwrite them by
  // enqueuing more work.
  iree_loop_async_op_t op = ring->ops[slot];

  // We pass the callbacks a loop that has the reentrancy bit set.
  // This allows iree_loop_async_ctl to determine whether it needs to alloc
  // more stack space.
  iree_loop_t loop = iree_loop_async_reentrant(ring);
  ...
  {
  case IREE_LOOP_COMMAND_CALL:
    iree_loop_async_run_call(loop, op.params.call);
    break;
  case IREE_LOOP_COMMAND_DISPATCH:
    iree_loop_async_run_dispatch(loop, op.params.dispatch);
    break;
  case IREE_LOOP_COMMAND_WAIT_UNTIL:
    iree_loop_async_run_wait_until(loop, op.params.wait_until);
    break;
  case IREE_LOOP_COMMAND_WAIT_ONE:
    iree_loop_async_run_wait_one(loop, op.params.wait_one);
    break;
  case IREE_LOOP_COMMAND_WAIT_ANY:
    iree_loop_async_run_wait_any(loop, op.params.wait_multi);
    break;
  case IREE_LOOP_COMMAND_WAIT_ALL:
    iree_loop_async_run_wait_all(loop, op.params.wait_multi);
    break;
  default:
    break;
  }
...
static void iree_loop_async_abort_all(iree_loop_async_ring_t *ring)
{
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  while (!iree_loop_async_ring_is_empty(ring))
  {
    uint8_t slot = ring->read_head;
    ring->read_head = (ring->read_head + 1) & IREE_LOOP_ASYNC_RING_MASK;
...
static void iree_loop_async_emit_error(iree_loop_t loop,
                                       iree_status_t status)
{
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  iree_loop_async_ring_t *ring = (iree_loop_async_ring_t *)loop.self;
  ...
  iree_loop_async_abort_all(ring);

  IREE_TRACE_ZONE_END(z0);
}

// Runs the |ring| until it is empty or an operation fails.
static iree_status_t iree_loop_async_run_all(iree_loop_async_ring_t *ring)
{
  IREE_TRACE_ZONE_BEGIN(z0);

  do
  {
    // Dequeue the next op and run it async.
    iree_loop_async_dequeue_and_run_next(ring);
  } while (!iree_loop_async_ring_is_empty(ring));
                                                  ...
                                                  void **inout_ptr)
{
  ...
  iree_loop_async_ring_t stack_ring;
  iree_loop_async_ring_initialize(status_ptr, &stack_ring);

  // Enqueue the initial command; we'll dequeue it right away but this keeps
  // the code size smaller.
  IREE_RETURN_IF_ERROR(iree_loop_async_enqueue(&stack_ring, command, params));

  // If the status is not OK then we bail immediately; this allows for sticky
  // errors that mimic the abort behavior of an actual loop. Async loops never
  ...
  if (iree_status_is_ok(*status_ptr))
  {
    ...
    return iree_loop_async_run_all(&stack_ring); // tail
  }
  else
  {
    // Abort all ops.
    iree_loop_async_abort_all(&stack_ring);
    return iree_ok_status();
  }
}
                                  ...
                                  const void *params, void **inout_ptr)
{
  ...
  iree_loop_async_storage_t *storage = (iree_loop_async_storage_t *)self;
  iree_loop_async_ring_t *ring = (iree_loop_async_ring_t *)storage->opaque;
  ...
  iree_loop_async_ring_initialize(&storage->status, ring);

  IREE_RETURN_IF_ERROR(iree_loop_async_enqueue(ring, command, params));
  ...
  {
    // Run until the ring is empty or we fail.
    return iree_loop_async_run_all(ring); // tail
  }
  else
  {
    // Abort all ops.
                                                   ...
                                                   void **inout_ptr)
{
  ...
  iree_loop_async_ring_t *ring = (iree_loop_async_ring_t *)self;
  return iree_loop_async_enqueue(ring, command, params); // tail
}
